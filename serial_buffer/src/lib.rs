...
impl SerialBuffer {
    ...
    /// Replace the downstream writer without touching the buffered contents.
    ///
    /// Used to retarget the buffer at a freshly-connected client, or at a
    /// discarding sink (e.g. `std::io::sink()`) on disconnect, while keeping
    /// the accumulated history ring intact so it can be replayed to the next
...
mod tests {
    ...
    // Output produced while a client is disconnected is retained and delivered
    // to the next client (reconnect replay).
    #[test]
    fn history_retained_across_reconnect() {
        let write_out = Arc::new(AtomicBool::new(false));
        let mut buf = SerialBuffer::new(Box::new(std::io::sink()), write_out.clone());
